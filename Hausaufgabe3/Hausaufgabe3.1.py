...
#text1
# Hier finde ich die durchschnittliche Wortlaenge fuer text1
...
# Mit text1.split() , teile ich den text1 in einzelne Woerte mit allen moeglichen Whitespaces als separator auf
# Mit der for-Schleife iteriere ich die ganze Liste der Woerter und addiere die Laengedes einzelnen Wortes auf sum_letters
...
#text2
# Hier finde ich die durchschnittliche Wortlaenge fuer text2
# Zunaechst definiere ich sum_letters , und initiziere die mit 0 , diese Variable wird die gesamte Anzahl der Buchstaben enthalten
# Mit text2.split() , teile ich den text2 in einzelne Woerte mit allen moeglichen Whitespaces als separator auf
# Mit der for-Schleife, rufe ich zunaechst range(len(words)) auf , das wird eine Sequenz von 0 , 1 , ..., len(words)-1 erzeugen
# dann iteriere ich über diese Sequenz , in jedem Iterationschritt , wird i = 0 , ... , len(words)-1 sein , und dann mit words[i]
# greife ich auf die einzelnen Woerter zu und mit sum_letters += len(words[i]) addiere ich die Laenge des einzelnen Wortes auf sum_letters
...
#text3
# Hier finde ich die durchschnittliche Wortlaenge fuer text3
# Zunaechst definiere ich sum_letters , und initiziere die mit 0 , diese Variable wird die gesamte Anzahl der Buchstaben enthalten
# Mit text3.split() , teile ich den text3 in einzelne Woerte mit allen moeglichen Whitespaces als separator auf
# Mit der for-Schleife, rufe ich zunaechst enumerate(words) auf , damit ermoegliche ich die Iteration mit Index und Element,
...
# 1) ein ohne dummy
# 2) anderes mit dummy
# Hier ich ziehe Implementation ohne dummy vor (Aber Sie koennten die Aufgabestellung präziser formulieren.)
# Anzahl der Bigramme (ohne dummy) = Anzahl der Woerter - 1
# Anzahl der Bigramme (mit dummy) = Anzahl der Woerter + 1
#text1
# Mit words = text1.split() , teile ich text1 in einzelne Woerter mit alle moeglichen whitespaces als Separator auf
# und weise das Ergebnis der Variable words zu.
...
#text2
# Mit words = text2.split() , teile ich text2 in einzelne Woerter mit alle moeglichen whitespaces als Separator auf
# und weise das Ergebnis der Variable words zu.
...
#text3
# Mit words = text3.split() , teile ich text3 in einzelne Woerter mit alle moeglichen whitespaces als Separator auf
# und weise das Ergebnis der Variable words zu.
# Mit mit count_bigramms_1 = len(words) -1 , bestimme ich die Anzahl der bigramme (len(words) gibt die Anzahl der Woerter
# zurueck)
words = text3.split()
...
# largest_word_1 = "" , damit definiere ich die Variable und inizialisire die mit einem leerem String. Diese Variable wird
# das laengste Wort enthalten
# words = text1.split() , teilt text1 in einzelne Woerte auf .
# in der for-Schleife iteriere ich über die einzelnen Woerter der words, und vergleiche die Laenge des Wortes mit der Laenge
# der largest_word_1 , wenn das Wort groesser ust als largest_word_1 , dann weise ich dieses Wort largest_word_1 zu.
largest_word_1 = ""
words = text1.split()
for word in words:
...
# largest_word_2 = "" , damit definiere ich die Variable und inizialisire die mit einem leerem String. Diese Variable wird
# das laengste Wort enthalten
# words = text2.split() , teilt text2 in einzelne Woerte auf .
# in der for-Schleife iteriere ich über die einzelnen Woerter der words, und vergleiche die Laenge des Wortes mit der Laenge
# der largest_word_2 , wenn das Wort groesser ist als largest_word_2 , dann weise ich dieses Wort  largest_word_2 zu.
largest_word_2 = ""
words = text2.split()
for word in words:
...
# largest_word_3 = "" , damit definiere ich die Variable und inizialisire die mit einem leerem String. Diese Variable wird
# das laengste Wort enthalten
# words = text3.split() , teilt text3 in einzelne Woerte auf .
# in for-Schleife iteriere ich über die einzelnen Woerter der words, und vergleiche die Laenge des Wortes mit der Laenge
# der largest_word_3 , wenn das Wort groesser ist als largest_word_3 , dann weise ich dieses Wort der largest_word_3 zu.
largest_word_3 = ""
words = text3.split()
for word in words:
...
# words = text1.split() -> hier teile ich text1 in einzelne Woerte auf
...
# Hier konstruiere ich die Liste on-the-fly und sortiere die mit reverse=True
...
# Hier gebe ich die Anzahl der Bigramme aus in Ihnem gewuenschten Format
print("Bigramme: ", count_bigramms_1, count_bigramms_2, count_bigramms_3)

# Hier vergleiche ich average_word_length von text1, text2 , text3
# da sorted_averages ein umgekehrt-sortierte Liste der 3 Elemente ist => das erste Element ist das groesste ,
